...
import com.lingona.es4j.domainpersistence.api.*;
...
public class EventStoreRepository implements IRepository, Closeable {
	...
	synchronized (this.streams) {
	    ...
	    for (Entry<UUID, IEventStream> entry : this.streams.entrySet()) {
                throw new UnsupportedOperationException("Not yet implemented"); //entry.getValue().close();
            }
    ...
    @Override
    public <TAggregate extends IAggregate> TAggregate getById(Class<TAggregate> clazz, UUID id) { // where TAggregate : class, IAggregate // virtual
        return getById(clazz, id, Integer.MAX_VALUE);
    }

    @Override
    public <TAggregate extends IAggregate> TAggregate getById(Class<TAggregate> clazz, UUID id, int versionToLoad) { // where TAggregate : class, IAggregate // virtual

        Snapshot     snapshot  = this.getSnapshot(id, versionToLoad);
        IEventStream stream    = this.openStream(id, versionToLoad, snapshot);
	TAggregate   aggregate = this.getAggregate(clazz, snapshot, stream);
    ...
    private <TAggregate extends IAggregate> TAggregate getAggregate(Class<TAggregate> clazz, Snapshot snapshot, IEventStream stream) {
        ...
        return this.factory.build(clazz/*typeof(TAggregate)*/, stream.getStreamId(), memento);
    }
