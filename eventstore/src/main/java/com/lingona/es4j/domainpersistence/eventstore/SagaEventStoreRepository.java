...
import com.lingona.es4j.domainpersistence.api.HeaderUpdater;
import com.lingona.es4j.domainpersistence.api.ISagaRepository;
import com.lingona.es4j.domainpersistence.api.PersistenceException;
...
public class SagaEventStoreRepository implements ISagaRepository, Closeable {
    ...
    @Override
    public <TSaga extends ISaga> TSaga getById(Class<TSaga> clazz, UUID sagaId) { // where TSaga : class, ISaga, new()
        return buildSaga(clazz, this.openStream(sagaId));
    }
    ...
    private IEventStream openStream(UUID sagaId) {
     ...
     static private <TSaga extends ISaga> TSaga buildSaga(Class<TSaga> clazz, IEventStream stream) { // where TSaga : class, ISaga, new()

        if(true)throw new UnsupportedOperationException("Not yet implemented"); TSaga saga = null; //new TSaga();
